...
void main() {
  group('ShoppingListService - calculateProjectedIngredients', () {
    ...
    test('returns correctly formatted data structure (empty case)', () async {
      ...
      expect(result.isEmpty, isTrue,
          reason: 'Should return empty map when no meals planned');
    });
    ...
    test('groups ingredients by category', () async {
      ...
      // Verify grouping
      expect(result.keys.length, equals(3), reason: 'Should have 3 categories');
      expect(result.containsKey('vegetable'), isTrue);
    ...
    test('handles date range correctly', () async {
      ...
      final recipe1 =
          Recipe(id: 'recipe-1', name: 'Recipe 1', createdAt: DateTime.now());
      final recipe2 =
          Recipe(id: 'recipe-2', name: 'Recipe 2', createdAt: DateTime.now());
      final recipe3 =
          Recipe(id: 'recipe-3', name: 'Recipe 3', createdAt: DateTime.now());
      mockDb.recipes[recipe1.id] = recipe1;
      mockDb.recipes[recipe2.id] = recipe2;
      ...
      final mpir1 = MealPlanItemRecipe(
          id: 'mpir-1', mealPlanItemId: 'mpi-1', recipeId: recipe1.id);
      final mpir2 = MealPlanItemRecipe(
          id: 'mpir-2', mealPlanItemId: 'mpi-2', recipeId: recipe2.id);
      final mpir3 = MealPlanItemRecipe(
          id: 'mpir-3', mealPlanItemId: 'mpi-3', recipeId: recipe3.id);
      ...
      expect(result['vegetable']![0]['quantity'], equals(300.0),
          reason:
              'Should include day1 (100g) + day3 (200g) = 300g, exclude day7 (300g)');
    });
    ...
    test('handles multi-recipe meals', () async {
      ...
      final mainRecipe =
          Recipe(id: 'recipe-1', name: 'Main Dish', createdAt: DateTime.now());
      final sideRecipe =
          Recipe(id: 'recipe-2', name: 'Side Dish', createdAt: DateTime.now());
      ...
      expect(result['grain']![0]['quantity'], equals(250.0),
          reason:
              'Should aggregate rice from main (150g) + side (100g) = 250g');
    });
    ...
    test('preserves existing aggregation logic', () async {
      ...
      expect(result['vegetable']![0]['quantity'], equals(1.5),
          reason:
              'Existing logic: 500g + 1kg = 1500g = 1.5kg (auto-converted)');
  ...
  group('ShoppingListPreviewBottomSheet - Widget Tests', () {
    ...
    Widget buildTestWidget(
        Map<String, List<Map<String, dynamic>>> groupedIngredients) {
      return MaterialApp(
    ...
    testWidgets('displays quantities correctly', (WidgetTester tester) async {
      ...
      expect(find.text('1½ kg'),
          findsOneWidget); // QuantityFormatter formats 1.5 as 1½
    });

    testWidgets('shows empty state when no ingredients',
      ...
      expect(
          find.text('No meals planned - nothing to preview'), findsOneWidget);
    });
    ...
    testWidgets('handles long ingredient names', (WidgetTester tester) async {
      ...
      expect(find.text('Extra Large Organic Heirloom Tomato From Local Farm'),
          findsOneWidget);
  ...
  group('Edge Case Tests', () {
    ...
    test('edge case - handles ingredients with missing data gracefully',
        () async {
      // Setup: Create ingredient
      final tomato = Ingredient(
        id: 'ing-1',
      ...
      // Create recipe
      final recipe =
          Recipe(id: 'recipe-1', name: 'Recipe', createdAt: DateTime.now());
      mockDb.recipes[recipe.id] = recipe;
      ...
      final mpir = MealPlanItemRecipe(
          id: 'mpir-1', mealPlanItemId: 'mpi-1', recipeId: recipe.id);
