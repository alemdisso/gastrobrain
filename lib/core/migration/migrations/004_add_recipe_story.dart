...
/// Adds a `story` column to the recipes table.
///
/// Stores personal and cultural narrative context for a recipe — distinct from
/// the `notes` field which serves practical culinary observations. Supports
...
class AddRecipeStoryMigration extends Migration {
  ...
  Future<void> up(DatabaseExecutor db) async {
    ...
    if (cols.any((r) => r['name'] == 'story')) return;
    await db.execute(
      "ALTER TABLE recipes ADD COLUMN story TEXT DEFAULT ''",
    );
  }
  ...
  Future<bool> validate(DatabaseExecutor db) async {
    ...
    return result.any((r) => r['name'] == 'story');
  }
}
