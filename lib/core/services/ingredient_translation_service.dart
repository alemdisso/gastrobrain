...
class IngredientTranslationService {
  static const String _translationDataAsset =
      'assets/reviewed_ingredients_pt.csv';
  ...
  Future<List<Map<String, String>>> _loadTranslationData() async {
    try {
      ...
      throw GastrobrainException(
          'Failed to load translation data: ${e.toString()}');
    }
  }
    ...
    try {
      ...
      for (final translation in translations) {
        try {
          ...
          // Parse unit
          final unit =
              unitStr.isNotEmpty ? MeasurementUnit.fromString(unitStr) : null;
          ...
          errorCount++;
          errors
              .add('Failed to update ${translation['name']}: ${e.toString()}');
        }
      }
